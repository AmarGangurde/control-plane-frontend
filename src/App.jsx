...
const Layout = ({ children }) => {
  const { user } = useAuth();
  return (
    <div className="min-h-screen bg-[#020617] text-slate-50 font-sans selection:bg-blue-500/30 grad-surface">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-12">
            <div className="flex items-center gap-3 group cursor-pointer">
              <div className="bg-white p-1.5 rounded-xl shadow-2xl shadow-white/10 group-hover:scale-110 transition-transform duration-500">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-bold text-blue-500 tracking-[0.2em] uppercase leading-none mt-1">
                  Control Plane
                </span>
              </div>
            </div>
            <div className="hidden md:flex items-center gap-1 p-1 bg-white/5 rounded-2xl border border-white/5">
              <NavLink
                to="/dashboard"
                className={({ isActive }) => `px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
              >
                Apps
              </NavLink>
              <NavLink
                to="/databases"
                className={({ isActive }) => `px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${isActive ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
              >
                Databases
              </NavLink>
              <NavLink
                to="/billing"
                className={({ isActive }) => `px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
              >
                Billing
              </NavLink>
            </div>
          ...
          <div className="flex items-center gap-6">
            {user?.email && (
              <div className="flex flex-col items-end hidden lg:flex">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Operator State</span>
